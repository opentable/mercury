...
describe('mapSmartlingFiletype.map()', () => {
    it('should correctly map file extensions to the correct Smartling file type', () => {
        ...
        expect(
            mapSmartlingFiletype.map(
                '<!-- smartling.instruction_comments_enabled = on --><resources><string name="test_string">Test XML</string><!-- smartling.instruction_comments_enabled = off --></resources>',
                'xml'
            )
        ).to.equal('xml');
